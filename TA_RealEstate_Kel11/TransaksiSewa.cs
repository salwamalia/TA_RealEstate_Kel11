...
namespace TA_RealEstate_Kel11
{
    public partial class TransaksiSewa : Form
    {
        ...
        private string IDOtomatis()
        {
            ...
            string sqlQuery = "SELECT TOP (1) MAX(RIGHT (idTSewa,2))+1 AS idTSewa FROM tPenyewaan";
            SqlCommand cmd = new SqlCommand(sqlQuery, myConnection);
            SqlDataReader dr = cmd.ExecuteReader();

            while (dr.Read())
            {
                if (dr["idTSewa"].ToString() == "")
                {
                    autoid = 1;
                }
                else
                {
                    autoid = Int32.Parse(dr["idTSewa"].ToString());
                }
            }
        ...
        private void btnSimpan_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtIDSewa.Text == "" || dateTimePicker1.Text == "" || cbProperty.Text == "" || cbClient.Text == "" || txtTotal.Text == "")
                {
                ...
                else
                {
                    ...
                    SqlConnection myConnection = connection.Getcon();
                    myConnection.Open();
                    SqlCommand insert = new SqlCommand("sp_InsertPenyewaan", myConnection);
                    insert.CommandType = CommandType.StoredProcedure;

                    insert.Parameters.AddWithValue("@idTSewa", txtIDSewa.Text);
                    ...
                    myConnection1.Open();
                    SqlCommand insert1 = new SqlCommand("sp_InsertDetailPenyewaan", myConnection1);
                    ...
                    myConnection1.Close();
                    clear();
                    MessageBox.Show("Save Sucessfully", "Add Penyewaan", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlConnection myConnection = connection.Getcon();
                myConnection.Open();
                SqlCommand cmd = new SqlCommand("sp_UpdatePenyewaan", myConnection);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@idTSewa", txtIDSewa.Text);
                ...
                SqlConnection myConnection1 = connection.Getcon();
                myConnection1.Open();
                SqlCommand cmd2 = new SqlCommand("sp_UpdateDetailPenyewaan", myConnection1);
        ...
        private void btnHapus_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (MessageBox.Show("Lanjut ingin Menghapus?", "Delete Transaksi", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    //DELETE PEMBELIAN
                    SqlConnection myConnection = connection.Getcon();
                    SqlCommand delete = new SqlCommand("sp_DeletePenyewaan", myConnection);
                    delete.CommandType = CommandType.StoredProcedure;

                    delete.Parameters.AddWithValue("idTSewa", txtIDSewa.Text);
        ...
        void LoadData()
        {
            try
            {
                ...
                SqlConnection con1 = connection.Getcon();
                con1.Open();
                SqlCommand cmd1 = new SqlCommand("SELECT * FROM tDetailPenyewaan", con1);
                SqlDataReader dr1 = cmd1.ExecuteReader();
                SqlDataReader sqlDataReader1 = dr1;
                DataTable dataTable1 = new DataTable();
                dataTable1.Columns.Add("ID Transaksi");
                ...
                {
                    DataRow row1 = dataTable1.NewRow();
                    row1["ID Transaksi"] = sqlDataReader1["idTSewa"];
        ...
        private void dgTransaksi_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                txtIDSewa.Text = dgDetailTransaksi.Rows[e.RowIndex].Cells[0].Value.ToString();
                ...
                //cicilan
                if (!(String.IsNullOrEmpty(dgDetailTransaksi.Rows[e.RowIndex].Cells[5].Value.ToString())))
                {
                    for (int i = 0; i < cbCicilan.Items.Count; i++)
                    {
                        cbCicilan.SelectedIndex = i;
                        if (dgDetailTransaksi.Rows[e.RowIndex].Cells[5].Value.ToString() == cbCicilan.SelectedItem)
                        {
                            break;
                        }
                    }
                ...
                txtperBulan.Text = dgDetailTransaksi.Rows[e.RowIndex].Cells[6].Value.ToString();
                txtDP.Text = dgDetailTransaksi.Rows[e.RowIndex].Cells[7].Value.ToString();
                txtTotal.Text = dgDetailTransaksi.Rows[e.RowIndex].Cells[8].Value.ToString();
