...
{
    public partial class PembayaranPembelian : Form
    {
        ...
        private string IDOtomatis()
        {
            ...
            string sqlQuery = "SELECT TOP (1) MAX(RIGHT (idBBeli,2))+1 AS idBBeli FROM tPembayaranPembelian";
            SqlCommand cmd = new SqlCommand(sqlQuery, myConnection);
            SqlDataReader dr = cmd.ExecuteReader();

            while (dr.Read())
            {
                if (dr["idBBeli"].ToString() == "")
                {
                    autoid = 1;
                }
                else
                {
                    autoid = Int32.Parse(dr["idBBeli"].ToString());
                }
            }
        ...
        private void dgTransaksi_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                dgTransaksi.Rows[e.RowIndex].Cells[1].Value.ToString();
